...
残っているもの        意味
====================  ==========================================
カタカナ外来語        **正常**。外来語は開いたクラスで，辞書に全部は入らない
...
本スクリプトは促音・拗音の小書き，``ヰヱヲ``，長音符の揺れをまとめたキーで
語を分類し，2種類以上の綴りを持つ組を出す。
...
def fold_katakana(s: str, loose: bool = False) -> str:
    ...
    **これは比較のためのキーであって，本文を書き換えるわけではない。**
    ...
    ``loose`` は語頭以外の ``アイウエオ`` も除く。長音を母音字で
    ...
    # ヰ→イ とまとめると「ヰスキー」と「ウイスキー」が別のキーになってしまう。
...
def unknown_pos_profile(tsvdir: str) -> dict:
    ...
    **解析が正しく働いている徴候**である。外国の人名・地名は
    二重に開いたクラス（固有名詞であり，かつ外来語である）なので，
...
def main() -> int:
    ...
    if args.tsv:
        ...
        if posmap:
            ...
            print('  働いている徴候である。** 外国の人名・地名は二重に開いたクラス')
            print('  （固有名詞かつ外来語）なので，どの辞書にも入らない。')
            print('  それが**1語のまとまりとして**未知語に立てられていることは，')
