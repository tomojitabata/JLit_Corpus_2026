...
=======================
記述統計と文体計量。埋め込みやトピックモデルに進む前に，**素朴な頻度で
何が見えるか**を確定させておく。ここで見えないものが埋め込みで見えたときは，
たいてい前処理の副作用である。
...
def main() -> int:
    ...
    # 以前は period を 'unknown' に落として集計に混ぜていた。すると
    ...
    # 正しく罰する。df だけでは「広く薄く出るが1作に集中している」語を
