...
# 他の帯と同じ規模にはならない。**標本の量が足りない帯でモデルを学習すると，
...
def style_class(bungo: float, kogo: float) -> str:
    ...
    **閾値は測定法とともに改めた。** 以前は絶対値で切っていたが，
    その口語標識は ``である``＝``で``＋``ある`` のような複合形を
    数え落としており，実際の 1/8 程度に過小評価されていた。
    過小評価された値に合わせた閾値をそのまま使うと，口語作品が
    軒並み「過渡的」に分類される。

    いまは**比**を主に使う。絶対値の条件を残してあるのは，
