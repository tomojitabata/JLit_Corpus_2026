...
if CORPUS_V1.exists():
 ...
 ('md', r'''## 6. 設計目標 — 「バランスが良い」を数値にする

「バランスの良いコーパス」は，**数値にしない限り検証できない**。どの作家が
何割まで許されるのかを決めずに増補すると，増やしたことで却って偏る。

そこで目標を `config/design_targets.yaml` に書いてある。

| 何を | 目標 | なぜ |
|---|---|---|
| 1作家の語数比 | 12 % 以下 | G² も PCA も word2vec も「誰が書いたか」を見ない。2割を占める作家がいれば，その文体が「時代の特徴」として出てくる |
| 1作品の語数比 | 8 % 以下 | 同上。長篇1点が時代を代表してしまう |
| 区分内の最大作家 | 40 % 以下 | 時代スライスが一人の作家になっていないか |
| 1スライスの語数 | 50万語以上 | 少ないスライスのモデルだけ語彙も近傍も貧しくなる |
| スライスの最大/最小 | 3 倍以内 | 「時代が違う」のか「標本が少ない」のかを区別するため |
| 各区分の女性作家 | 2人以上 | 性別と時代の交絡を減らす |

⚠ **目標は達成義務ではない。** 青空文庫に無いものは入らない。保護期間中の
作家（野上弥生子・平林たい子・佐多稲子・尾崎翠）は，どれだけ望んでも
入れられない。**埋まらない穴を穴として報告し続ける**のがこの表の役目である。

下のセルで，現状との差が出る。'''),
 ('code', r'''# 設計目標と現状の差。**達していない項目が出るのが正しい。**
# 目標値を変えるときは config/design_targets.yaml を書き換え，
# logs/ に日付つきの写しを残すこと。
run_script('check_balance.py')'''),
 ('md', r'''### 2026-09-26 の設計見直し

この授業のコーパスは，2026-09-26 に次の偏りが見つかって手を入れた。
**どれも「数えるまで見えなかった」ものである。**

1. **夏目漱石 4 点（語数 4.7 %）に対し森鴎外 7 点（11.8 %）。** 文学史的な
   位置づけに対して明らかに不均衡で，漱石を 9 点に増補することにした。
   うち2点（『行人』『道草』）は大正期に入るので，**大正期の鴎外 32 %
   という偏りも同時に薄まる**。一手で二つの穴が埋まる。
2. **明治前期は3点すべて福沢諭吉**（語数 1.1 %）。時代区分ではなく一人の
   作家である。→ 学習では中期に畳む（`period5`）。
3. **昭和戦後は語数の 50 % が宮本百合子**，59 % が女性。**性別と時代が
   交絡している**ので，「女性作家の語彙」として出てくるものの多くは
   「戦後の語彙」かもしれない。→ 戦後の男性作家を増補。
4. **前期〜中期の女性作家は樋口一葉 1 人**。→ 明治期の女性作家を探す。

増補の候補は `metadata/expansion_candidates.csv`，取得の指示は
`config/corpus_manifest.tsv` にある。'''),
 ('md', r'''## 7. このステップの課題
...
# 共用 iMac では代表者1名が実行し，data/aozora/xhtml を共有すること。
...
2. v1 で `※` になっていた箇所を **3つ**選び，青空文庫の原文から復元した文字と，
   その語の意味を示せ（辞書を引くこと）。3つのうち少なくとも1つは
   **面区点を持たない注記**（`※［＃小書き片仮名ヲ、160-9］` の形）から選ぶこと。
3. XML から会話文比率を計算し，ジャンル別に比較した図を1枚作れ。
...
（1.575 > 1.560）。刻んで稼いだのではない，と言える。この確認をせずに
...
他の受講生と同時に使うときは，`PORT` を各自変えること（8765, 8766, …）。"""),
 ('code', r'''# ---- 画面を立てる ----------------------------------------------------
...
if port_open(PORT) and is_kwic(PORT):
    ...
    # 自分の過程ではない何かが港を使っている（前回の残り・他の受講生）
...
3. `--chunk` を 1000 / 2000 / 5000 と変え，チャンク数の分布がどう変わるか図示せよ。
   トピックモデルにとってどれが望ましいか，理由とともに述べよ。
...
4. 検証ログを読み，上の表にしたがって**各警告を「直す／直さない」に分類**し，
   直さないものについてはその理由を1行で書け。FATAL 0 は目標ではない。
5. `narration_excludes_embedded` を `true` にして `04` を再実行し，
   『こころ』の `narration` の字数がどれだけ減るかを示せ。
   どちらの設定が自分の問いに適しているかを述べよ。
6. **（発展）辞書を2つ用意して `12_dict_compare.py` を回し**，
   未知語率・平均語長・境界一致率を層別に出せ。
   `dict_disagreements.csv` から食い違いを**3例選んで原文と照らし**，
   どちらの切り方が自分の問いに適しているかを述べよ。
...
どちらが大きいか。それは何を意味するか。'''),
 ('code', r'''if need(p, 'この分析のスクリプトを走らせるセルを先に実行すること'):
    # 鍵の 0 埋めと file_v1 の欠損は work_rows() が面倒を見る。
...
4面のうち **`period` の面だけ1色相の濃淡**（薄い水色 → 濃紺の5段）で塗る。
理由は Step 1 の散布図と同じである。

- 時代は**順序のある変数**である。色相環の8色で塗ると，明治中期が青・
...
if need(p, 'この分析のスクリプトを走らせるセルを先に実行すること'):
    ...
    # それを黙って塗ると，62 点が「初出年不明」の灰色で並ぶ（実際に起きた）。
...
2. Delta の最近傍一致率（作家 / 時代 / ジャンル）を報告し，
   このコーパスで通時的主張をするときの注意点を述べよ。
3. 固有名詞を除いた特徴語リストを作り，除く前と比較せよ。
4. MFW の語数（100 / 300 / 1000）を変えると PCA の布置がどう変わるか確かめよ。
5. `speech_markup` が `full` でない作品を除いた場合と含めた場合とで，
   会話文比率と第1主成分の相関がどう変わるかを示せ。**該当作品は
   `conversion_report.csv` から自分で数えること**（コーパスの増補で
   件数は変わる）。
6. **culling（4.1–4.3）を報告の形に書け。** 閾値を1つ宣言し，
   時代ごとに「残存率・重なり@15・入替@15」の表を出し，
   *culling の有無で結論が変わる時代*があるかを述べよ。
...
| 時代区分（6区分） | 語数 | 判断 |
|---|---:|---|
| 明治前期（〜1886） | 67,381 | **単独では学習不可**。しかも3点とも福沢諭吉 |
| 明治中期（1887–1899） | 703,600 | 可 |
| 明治後期（1900–1911） | 674,022 | 可 |
| 大正（1912–1925） | 1,395,723 | 可 |
| 昭和戦前（1926–1944） | 2,133,529 | 可 |
| 昭和戦後（1945–） | 1,236,768 | 可 |

（分析対象 101 点・6,211,023 語。2026-09-26 の実測。）

明治前期だけが桁違いに小さい。**これは時代区分ではなく一人の作家である。**
青空文庫の所蔵そのものが薄いので，増補してもこの帯が他に並ぶことはない。

そこで**前期を中期に畳んだ5区分 `period5` を学習の既定**にする。

| | 記述統計（Step 1・4） | 学習（Step 6・この Step） |
|---|---|---|
| 使う列 | `period`（6区分） | **`period5`（5区分）** |
| 根拠 | 文学史の区切り | **標本の量** |

⚠ **根拠の違う2つの区分を，列を分けて持っている。** 6区分は文学史の
区切りだが，5区分は「標本が足りないから畳んだ」ものである。論文で
スライスの切り方を述べるときは，**どちらの根拠で切ったのかを書く**こと。

目標値（1スライス50万語以上・最大最小3倍以内）は
`config/design_targets.yaml` にあり，`scripts/check_balance.py` が現状との
差を報告する。2026-09-26 時点では最小 67万語で下限は満たすが，最大最小が
3.2 倍で目標をわずかに超えている（漱石の増補で明治後期が 100万語に届けば
2.1 倍に収まる見込み）。

対照として，より粗い切り方も残してある。

- `slice4` … 明治（〜1911）／大正／昭和戦前／昭和戦後 の4区分
- `slice2` … 戦前（〜1944）／戦後 の2区分（最も頑健）

**5区分で見えた変化は，必ず `slice2` でも確かめること。** 細かく切るほど
スライスあたりの語数は減り，変化量は乱数に近づく。'''),
 ('code', r'''DS = ROOT/'data'/'datasets'
...
# ---- 学習に使う5区分（period5）----------------------------------------
# 06 が索引に書く列だが，古い索引には無いので同じ規則でここでも引ける
# ようにしておく（規則は 00_build_metadata_v2.BAND5 と config/design_targets.yaml）。
BAND5 = {'1_明治前期(〜1886)': '1_明治前期〜中期(〜1899)',
         '2_明治中期(1887-1899)': '1_明治前期〜中期(〜1899)'}
if 'period5' not in ci.columns or ci['period5'].isna().all():
    ci['period5'] = ci.period.map(lambda p: BAND5.get(str(p), str(p)))
    print('[note] 索引に period5 が無いので period から引き直した。'
          '06_build_datasets.py を走らせ直すと索引にも入る。')
ci.loc[bad, 'period5'] = None
...
sl = ci.dropna(subset=['period5'])     # ← 以後のスライス学習はこれを使う

# **スライスの量を，学習の前に確かめる。** 語数が桁違いのスライスを
# 並べると，小さいスライスのモデルだけ語彙も近傍も貧しくなり，
# 「時代が違う」のか「標本が少ない」のかが区別できなくなる。
# 目標は config/design_targets.yaml（1スライス50万語以上・最大最小3倍以内）。
MIN_TOK, MAX_RATIO = 500_000, 3.0
g = (sl.groupby('period5')
       .agg(チャンク=('chunk_id', 'size'), 語数=('chunk_tokens', 'sum'),
            作品=('work_stem', 'nunique')))
g['目標50万語'] = np.where(g.語数 >= MIN_TOK, 'ok', '不足')
show(g.reset_index().rename(columns={'period5': '区分'}),
     caption='時代スライスの量（5区分）', fmt={'語数': '{:,.0f}'})
ratio = g.語数.max() / max(1, g.語数.min())
print(f'最大/最小 = {ratio:.1f} 倍' + ('' if ratio <= MAX_RATIO else
      f' ← 目標 {MAX_RATIO:.0f} 倍を超えている'))
if (g.語数 < MIN_TOK).any():
    print('[warn] 50万語に満たないスライスがある。増補が済むまでは，'
          '\n       粗い4区分（slice4）で回すほうが解釈しやすい'
          "（--slice を 'slice4' に替える）。")
print()
...
4. **コントロール実験**：時代ラベルをシャッフルして同じ手順を回し，
 ...
 ('code', r'''W2V = OUT/'w2v_period5'
# --runs は**種を変えて何回学習するか**。既定は 10（config/pipeline.yaml）。
...
run_script('08_word2vec_diachronic.py', '--chunks', DS/'chunks',
           '--index', DS/'chunks_index.csv', '--out', W2V, '--slice', 'period5',
...
ci_shuf['period5'] = rs.permutation(ci_shuf['period5'].values)
...
run_script('08_word2vec_diachronic.py', '--chunks', DS/'chunks',
           '--index', shuf_path, '--out', W2V_S, '--slice', 'period5',
           '--dim', 300, '--window', 3, '--min-count', 20, '--balance',
           '--runs', RUNS, tail=1500)
...
meta = load_meta()
# 鍵は work_rows() に作らせる。自前で f'{person_id}_{work_id}' と書くと
...
4. 「この語は意味が変化した」と言えるための条件を，自分の言葉で3つ挙げよ。
...
- 右：色相は `genre_main` の**2つだけ**にし，下位ジャンルは**印の形**で
...
色の勾配が見えなくなる。'''),
 ('code', r'''if need(p):
    ...
    # ------------------------------------------------------------------
    # 左：時代。**順序のあるものを categorical の8色で塗ってはいけない。**
    ...
    # そこで色相は genre_main の2つだけにして，下位ジャンルは**印の形**で
...
### 3. 方法
4つの方法のうち最低2つを使う。ハイパーパラメータと乱数種を明記する。
