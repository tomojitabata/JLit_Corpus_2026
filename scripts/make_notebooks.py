...
- 本体のラベル番号（例 2021-03）と，出力の先頭に出る**マシン名をリポートに控える**
...
説明できない列は，必ずあとで誤用される。'''),
 ('code', r'''# load_meta() は分析に使わない行（superseded / too_short）を除外して読む。
...
次のセルを実行し，**どの軸がいちばん偏っているか**を自分の言葉で述べてみよう。
...
> なので，リポートに貼るほうはベクタである。拡大して確かめたいときは，
...
これは**壊れているのではなく，前の工程がまだ実行されていない**という意味である。
...
ない。6段取るとどこかが 0.06 を下回り（0.05 台），隣の段と見分けられなくなる。
...
# ------------------------------------------------------------------
# **時代は順序のあるものである。** period をカテゴリーの8色で色分けすると，
# 明治中期が青で明治後期が黄…となり，隣り合う時代が隣り合う色にならない。
# そうすると「時代が下るにつれて点がどちらへ動くか」という，この図で
# 見たい当のものが読めなくなる。初出年を5段にまとめ，1色相の濃淡に割り当てる
...
# 上位10件の多くが重なって読めない。番号だけを打ち，名前は下の表で引く。
...
# 番号が付くのは上位10点だけである。残りの54点は「どの作品か」が
...
# 列全体が文字列として読まれ TypeError で止まる。数値化してから比べる。
...
メタデータではなく**テクスト本体**を見る。検証スクリプトを実行する前に，
まず素朴な方法で重複を探してみよう。
...
# 「そんな名前の変数は無い」と判定されて既定値に切り替わるだけなので気づきにくい。
...
print('**距離がほぼ 0 なら同一本文の疑い。** 次のセルで n-gram で確かめる。')'''),
 ('code', r'''# 8-gram シングル（shingle）による重複検出（99_validate.py の中核）
...
2. その偏りがあると，どのような研究上の問いに**答えられなくなる**か。具体的に1つ挙げること。
...
# ==========================================================================
L(2, 'コーパスの再構築 — 書誌の典拠とXMLマークアップ', [
 ('md', r'''# Step 2 コーパスの再構築 — 書誌の典拠と XML マークアップ
...
テクスト/XHTML の URL を1つの CSV で配布している。
...
show(q[cols].T.reset_index().rename(columns={'index':'項目'}),
     caption='『潜航艇「鷹の城」』の索引の記述（行と列を入れ替えた）')'''),
 ('md', r'''## 2. マニフェストによる取得
...
一覧は `data/aozora/unresolved.csv` にも書き出される。
...
`03_aozora2xml.py` の変換リポートには `gaiji_resolved` と
...
1. 正規化後テクスト（`data/plain/full/`）の `※` と `〓` をそれぞれ数えてみよう。
2. `※` が残っている作品を1つ選び，青空文庫の原文を見て，それが
   **(a) 外字マーカーの消し残し** なのか **(b) 底本そのものが使う記号**
   （編者注の印，伏字）なのかを判定してみよう。
3. (b) だった場合，それは欠陥か。なぜそう言えるか。

**ヒント**: 海野十三『敗戦日記』には `（※マリアナ基地からの…）` という
補注が 100 箇所あまりある。これは底本の編者（橋本哲男）が付けたもので，
**本文の一部**である。外字マーカーの消し残しではない。
なお，このファイルは `<div class="main_text">` を持たない旧形式なので，
`03_aozora2xml.py` は `<hr>` を手掛かりに奥付を除いている。奥付にも外字注記の `※` があるため，**奥付を除かないと (a) と (b) が
混ざって数えられない**。'''),
 ('md', r'''## 4. XML への変換
...
- Delta や doc2vec の最近傍が，同じ作品の別の巻になる。
  Step 4 の「最近傍が同じ作家である割合」は自明に上がり，指標として意味をなさない
...
`data/xml/_volumes/` に退避されるので，巻ごとの比較をしたくなれば戻せる。'''),
 ('code', r'''if need(XML, 'Step 2 の 03_aozora2xml.py を先に実行すること'):
    # まず確認だけ（--dry-run）。何が何にまとまるかを読んでから本番を実行する
    run_script('03b_merge_volumes.py', '--xml', XML,
               '--config', ROOT/'config'/'merge_volumes.tsv', '--dry-run')'''),
 ('code', r'''if need(XML, 'Step 2 の 03_aozora2xml.py を先に実行すること'):
    run_script('03b_merge_volumes.py', '--xml', XML,
               '--config', ROOT/'config'/'merge_volumes.tsv')'''),
 ('code', r'''# 変換リポート：外字がどれだけ復元できたか
p = XML/'conversion_report.csv'
if need(p, 'Step 2 の 03_aozora2xml.py を先に実行すること'):
    rep = pd.read_csv(p)
...
変換リポートには会話の標示に関する列が並ぶ。素朴な実装では出ない列である。
...
`speech_mark` が `『』` になる作品があることに注意することが重要である。島崎藤村『破戒』は
...
一律に `「」` を会話と見ると，この作品の会話が**丸ごと 0 件**になる。'''),
 ('code', r'''if need(p):
    ...
    show(rep.nlargest(6, 'said_unclosed')[cols],
         caption='閉じ括弧のない開き括弧が多い作品（標示の信頼性が下がる）')'''),
...
4. `scripts/find_unclosed_quotes.py` を `data/xml` にかけ，
   残った `unmarked_paragraph` を1つ選んで青空文庫の原文を確認し，
...
# ==========================================================================
L(3, '再構築からKWICへ — 踊り字の正規化・UniDic 解析・データセット再構築・KWICコンコーダンサ実装', [
 ('md', r'''# Step 3 再構築からKWICへ — 踊り字の正規化・UniDic 解析・データセット再構築・KWICコンコーダンサ実装
...
証拠にならない。**"""),
 ('code', r'''# ---- 索引を作る（05 を実行し直したら，これも作り直す）----------------
KW = ROOT/'data'/'kwic'
...
このステップで「何が作家由来で，何が時代由来か」を切り分ける感覚を養う。
...
if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    wp = pd.read_csv(p)
...
1. `6_Delta` で，作品X に**いちばん近い作品**と**いちばん近い作家プロファイル**を
   書き留めてみよう。両者は一致するか
2. `5_z差` で色の濃い（差の大きい）語を 3 つ挙げてみよう。それは作家の癖か，
   **語りの人称や登場人物名**など別の要因か
3. `7_語数Nを変える` の黄色のセルに 10・20・50 を入れ，順位の変わり方を見てみよう
4. `1_度数` の数字を 1 つ書き換え，影響がどのシートまで伝わるかを追ってみよう
...
最近傍が**同じ作家**である割合と，**同じ時代**である割合を比べてみよう。
どちらが大きいか。それは何を意味するか。'''),
 ('code', r'''if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    # キーの 0 埋めと file_v1 の欠損は work_rows() が処理する。
    # ここで自前に辞書を作ると，増補した作品の file_v1 が空なのでエラーで止まるか，
...
図どうしを同じ約束で見比べられなくなる。残りの3面は色相で色分けする。'''),
...
if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    ...
    # 補ってもなお空が残るなら，メタデータ側の空きと突き合わせて原因を確かめる。
    ...
    if n_nan:
        ...
        if n_nan > n_meta_blank:
            ...
            print('       07_descriptive_stats.py を今の版で実行し直すこと'
                  '（キーの 0 埋めの綴り違いを直してある）。')
            print('       それでも残るなら '
    ...
    # 作品数3点の明治前期を明治中期にまとめて5段にしてある（year_bands）。
...
if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    ...
    # 古い CSV には無いので，無ければ 4.1 以降が動かないことを先に知らせる。
    ...
    kt.index.name = '順位'
    # 有意語が 15 に満たない時代があると空行が並ぶので，全列が空の行は除く
    kt = kt[(kt != '').any(axis=1)]
...
⚠ **culling は G² の値を変えない。** 語を1つ除いても，他の語の
...
| 列 | 何を測るか | bursty なら |
|---|---|---|
| `df_all_prop` | 分析に使う全作品（108 点）のうち何割に出るか | **小さい** |
...
0–100 の百分率だけを `_pct` と綴り分ける。名前を見れば尺度が分かる。
...
| 指標 | 定義 | 読み |
|---|---|---|
| **残存率** | culling 後に残る有意語の割合 | 小さい＝その時代の特徴語に，多くの作品に広く出る語が少ない |
...
意味がない。** culling は語を除くだけで残った語の順序を変えないので，
...
if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    ...
    if 'dp_in' not in ky.columns:
        print('[warn] dp_in 列が無い。07 のセルを実行し直すこと。')
    else:
        rows = []
        for per in sorted(ky.period.unique()):
            a = ky[(ky.period==per)&(ky.G2>0)]
            ...
            rec['除外される語のdp_in中位'] = drop.dp_in.median() if len(drop) else np.nan
        ...
        show(mt, caption=f'culling（df < {CULL:.0%} を除外）の効き方',
             fmt={'残存率':'{:.1%}', '重なり@15':'{:.0%}', '重なり@50':'{:.0%}',
                  '残る語のdp_in中位':'{:.3f}', '除外される語のdp_in中位':'{:.3f}',
                  'ρ(G²,dp_in)':'{:+.2f}'})
...
- **弾かれないが bursty** … 多くの作品に1回ずつ出るうえに，1作品に
...
if need(p, 'この分析のスクリプトのセルを先に実行すること'):
    ...
    if 'dp_in' not in ky.columns:
        ...
        # このセル単独でも動くように，語幹→作家『作品』の表はここでも作る
        LAB4 = work_labels()
        def _w4(x):
            x = str(x)
            return LAB4.get(x, x)
...
`CULL` を `0.05` / `0.10` / `0.25` と変え，時代ごとに次を考えてみよう。

1. **重なり@15 が最も小さい時代**はどれか。その時代の
   「除外された語」の `top_work` を見て，**どの作品が効いていたか**を確かめてみよう。
2. 「入った語」（culling で浮上した語）を5語選び，それが
   **時代の特徴として説明できるか**を考えてみよう。説明できないなら，
   何を拾ってしまっているのか。
...
5. `speech_markup` が `full` でない作品を除いた場合と含めた場合とで，
   ...
   件数は変わる）。
6. **culling（4.1–4.3）を報告の形にまとめること。**閾値を1つ宣言し，
   時代ごとに「残存率・重なり@15・入替@15」の表を出し，
   *culling の有無で結論が変わる時代*があるかを述べること。
   変わる時代があれば，**その原因になっている作品を名指しすること**
...
2. ハイパーパラミター（window / dim / min_count / sg）の意味と影響を言える
...
word2vec に入る前に，**素朴な共起行列＋PPMI＋SVD** で同じことを試みる。
...
for w in ['女','心','国','汽車','恋','自由','戦争','機械','神']:
 ...
 ('md', r'''### 演習 1 — ハイパーパラミターの影響

`window` を 2 / 3 / 10 と変えて，同じ語の近傍がどう変わるかを見てみよう
（3 が本パイプラインの既定値である）。
...
# 学習の設定は本番と同じ（dim 300／window 3）。epochs だけ 15 に減らしてある。
...
print(f'Jaccard が 0.3 を下回る語は {len(weak)} 語{note}。'
      '**この語の近傍は解釈に耐えない。**')
print('対処は，min_count を上げる／コーパスを増やす／複数回の平均を取る，のいずれか。')'''),
...
2次元に圧縮して射影し，語のまとまりを見る。ただし**次元削減の手法は1つではない**。
同じ300語を**2つの方法**で射影して並べる。
...
何を守りたいかで選び，選んだ理由を報告に書く。"""),
 ('code', r'''# ---- 同じ300語を2つの方法で射影する ----------------------------------
WORD_PROJ = 'umap'      # 'umap' / 'tsne' / 'auto'（auto のときだけ t-SNE に切り替わる）
WORD_SEED = 20260920
...
show(pd.DataFrame([
     ...
     caption=f'同じ {len(sel)} 語を2つの方法で2次元に圧縮して射影した結果',
...
fig.suptitle(f'word2vec 空間（{X.shape[1]}次元・高頻度{len(sel)}語）を'
             f'2つの方法で2次元に圧縮して射影する', fontsize=12)
fig.tight_layout()
...
# **面ごとに座標が違う**ので coords で渡す（渡さないと2面めのポインタの判定範囲が
...
1. **同じ語**（たとえば「私」「汽車」「美しい」）が左右のどこに来るかを，
   対話版で追ってみよう。両方で端にある語，片方だけで端にある語を書き出す。
2. PCA の第1主成分の両端に来る語を10語ずつ並べ，**その軸が何か**を
   考えてみよう（頻度か，品詞か，語種か）。`vocab` の頻度と見比べる。
3. 上の表で ρ と信頼度がどちらに振れたかを確かめ，
   **「UMAP の絵で塊が2つに見えた」から言えること／言えないこと**を書き出してみよう。
4. `WORD_SEED` を変えて UMAP だけ描き直し，塊の位置がどれだけ動くかを見てみよう。
   **PCA はシードに依存しない**（同じ行列からは同じ答えが出る）。ここが
   「軸に意味がある」ことの実際的な意味である。"""),

 ('md', r"""## 4. 語彙のギャラクシー — 300次元を2次元に圧縮して射影し，眺める

近傍語の一覧は「1語ずつ」の確認である。**空間の全体像**を見たい。
300 次元は目で見られないので2次元に圧縮して射影する。ここでは **UMAP** を使う
（`GAL_PROJ` で切り替える。どちらで描いたかは図と HTML に記録される）。

### ⚠ UMAP で描けているか確かめる

`GAL_PROJ = 'umap'` が既定である。**使えないときは黙って t-SNE に切り替わらず，
...
切り分けと対処は診断スクリプトがすべて行う。**ノートブックと同じ
カーネルで**実行すること。
...
- **マーカーの形 = 意味／機能のカテゴリー**（`ANCHORS`）… 意味的なまとまり
...
そこでこの図は，**射影が近さをどれだけ歪めているか**も一緒に出す。
...
| `tokens_lemma` | 語彙素（見出し語）・**全品詞** | 活用と表記のゆれをまとめる。助詞・助動詞・代名詞も**空間に入る** |
| `tokens_content` | 語彙素のうち名詞・動詞・形容詞・副詞 | 機能語を除く。主題的な近さが出やすい |

**「表層形でないと空間が歪む」という直感には，半分だけ理由がある。**
分けて考える必要がある。

1. **機能語を除くこと**（content の問題）。ウィンドウ 3 で
   「彼 は 汽車 に 乗っ た」を見るとき，助詞を除くと隣接関係が
   「彼 汽車 乗る」に変わる。つまり**ウィンドウの意味が変わり，実効的に広くなる**。
   しかも「が／を／に」対「て／た」という**品詞を見分ける最強の手がかり**
   が消える。§4 で「文法的な関係もモデル化されている」ことを見せるなら，
   機能語を除くのはその手がかりを自ら捨てることになる。**この点で直感は正しい。**
2. **語彙素にまとめること**（lemma の問題）。こちらはまとめたほうがよい。
...
つまり**歪むのは「表層形でないから」ではなく「機能語を除いたから」**
...
「どこに何があるか」の手がかりとして，カテゴリーの典型語を**マーカーの形**で
目立たせる。名詞の意味カテゴリー（身体・親族・自然・器物・抽象語）に加えて，
**動詞・形容詞・副詞のカテゴリー**を入れ，さらに `tokens_lemma` で学習した
ときは**文語の助動詞・口語の助動詞・人称代名詞**の3群が加わる。
カテゴリーは `ANCHORS` に書いてあるので，**自分の問いに合わせて書き換えること**
（それがこのステップの課題でもある）。
...
if CMP_RUN:
    for st in STREAMS:
        d = TOK/st
        if not d.exists():
            print(f'[warn] {d} が無い（05 を --lemma-policy のまま実行すると'
                  '3つとも出る）。飛ばす。')
            continue
...
#   * 活用と旧仮名をまとめる → 低頻度の異形が減り，ベクトルが安定する
...
# カテゴリーの典型語。**マーカーの形**で示す。名詞の意味カテゴリーだけでなく，
# 動詞・形容詞・副詞のカテゴリーも入れる。**形は多くても11まで**にして，
...
# 頼らせない**。マーカーの形・図に直接置くカテゴリー名・表の3つで二重に示す。
...
# 「このカテゴリーは空間に無い」と誤解される。空間に入らなかった語は下で名指しする。
...
#            'tsne'  … t-SNE で描く
#            'auto'  … UMAP を試し，駄目なら t-SNE に切り替わる（理由は出す）
#
# ⚠ 既定を 'umap' にしてある。**黙って t-SNE に切り替わるのがいちばん悪い。**
# 図は出るが塊の見え方が変わるので，設定の問題を分析結果と読み違える。
# 原因の切り分けは  python3 scripts/check_umap.py  がすべて行う。
GAL_PROJ = 'umap'
...
if not POS_OK:
    print('[warn] 品詞の取得率が低いので**色分けはしない**。'
          'Step 3 を実行して data/tokens/tsv を作ると色が付く。')

cat = np.array([acat.get(w, '') for w in gal])
print(f'凡例のカテゴリーに入る語: {int((cat != "").sum())} 語／'
      f'その他 {int((cat == "").sum())} 語')
""" ),
 ('code', r"""# ---- 図（静止版 SVG ＋ 対話版 HTML）----------------------------------
# **色 = 品詞（文法）／マーカーの形 = カテゴリー（意味）** の二重の色分け。
...
# 凡例の語。形はカテゴリー，中の色は品詞（＝その語が実際に何と解析されたか）
cx, cy, cl = [], [], []
...
ax.set_xlabel('※ 軸に意味は無い。近さだけが意味を持つ（しかも近さも射影の結果である）'
              '／色＝品詞（文法的なまとまり）・マーカーの形＝カテゴリー（意味的なまとまり）')
ax.set_xticks([]); ax.set_yticks([])
for sp in ax.spines.values():
    sp.set_visible(False)
...
if h_pos:
    lg1 = ax.legend(handles=h_pos, title='色 ＝ 品詞', frameon=False,
                    ...
                    bbox_to_anchor=(1.01, 1.0), borderaxespad=0)
    ax.add_artist(lg1)
ax.legend(handles=h_cat, title='マーカーの形 ＝ カテゴリー', frameon=False,
          fontsize=8.5, title_fontsize=9, loc='upper left',
...
メモリはほとんど使わない。）まず**品詞**（文法），次に**カテゴリー**（意味）で測る。"""),
...
if POS_OK:
    ...
    print('window を 3 に狭めてあるので，この差は大きく出るはずである'
          '（ウィンドウを 10 に広げて測り直すと縮む。演習 1 の続きとしてやってみよう）。')
else:
...
for k in ANCHORS:
    ...
    rows.append({'カテゴリー': k, '語数': len(idx),
                 'カテゴリー内の平均類似度': r[0],
                 'カテゴリー外との平均類似度': r[1],
                 '差': r[0] - r[1],
                 '画面上の散らばり': spread,
                 '近傍保存の中央値': float(np.median(keep[idx]))})
show(pd.DataFrame(rows).sort_values('差', ascending=False),
     caption=f'**意味的なまとまり**：カテゴリーの凝集度'
             f'（原空間のコサイン類似度／全体平均 {allmean:+.3f}）',
     fmt={'カテゴリー内の平均類似度': '{:+.3f}', 'カテゴリー外との平均類似度': '{:+.3f}',
          '差': '{:+.3f}', '画面上の散らばり': '{:.2f}',
          '近傍保存の中央値': '{:.1f}'})
print('「差」が大きいカテゴリーは，**空間の中で実際にまとまっている**。')
print('小さいカテゴリーは，こちらが名前でまとめただけで，モデルはそう見ていない。')
print('画面上の散らばりが小さいのに差も小さい場合は，**射影がまとめて見せている**'
      'だけの可能性がある。原空間の数字のほうを信じること。')
print()
print('**2つの表を並べて読むこと。** 品詞でも差が出て，同じ品詞の中の'
      '意味カテゴリーでも差が出る——これが「文法的な関係と意味的な関係の'
      '両方がモデル化されている」ということの中身である。')"""),
 ('md', r"""### 演習 3 — ギャラクシーを自分の問いで色分けし直す

1. `ANCHORS` を**自分の問いのカテゴリー**に書き換えてみよう（8つまで＝マーカーの形の数）。
   例: 視覚語／聴覚語・和語／漢語・自然主義の語彙／プロレタリア文学の語彙。
   **品詞をまたぐカテゴリー**（例「戦争を語る語」＝名詞＋動詞＋形容詞）を
   立てると，色がばらけて形だけが揃う。これは
   「意味では揃うが文法では揃わない」カテゴリーの見え方である。
   書き換えたら**凝集度の表**を見て，そのカテゴリーが空間でまとまっているかを
   確かめる。まとまっていなければ，カテゴリーの立て方を疑う。
2. **近傍保存が低い語**（対話版で線が遠くまで伸びる語）を3語選び，
   なぜ2次元に収まらないのかを考えてみよう。多義語か，頻度が低いか，
   複数の文脈にまたがる語か。
3. `GAL_N` を 1000 / 3000 / 10000 と変え，**信頼度がどう変わるか**を記録してみよう。
   語を増やすと図は賑やかになるが，射影の歪みは増えるか減るか。
   （`W2V_WIN` を 10 にして学習し直し，**品詞の凝集度がどう動くか**も見てみよう。
   狭いウィンドウが文法を拾っているという説明が正しければ，差は縮むはずである。）
4. `min_dist` を 0.0 / 0.12 / 0.5 と変えると，塊の締まり方が変わる。
   **これは分析結果ではなく作図の設定である。** 締まった図を見て
   「クラスタがある」と言ってよいか，理由とともに考えてみよう。
5. ⚠ UMAP / t-SNE の座標は**シードに依存する**。`GAL_SEED` を変えて2回描き，
   同じ結論が言えるかを確かめてみよう。言えないなら，その結論は図の癖である。"""),
 ('md', r'''## 5. このステップの課題
...
解決策は Hamilton, Leskovec & Jurafsky (2016) の方法である。
...
# 06 の突合が外れると period は NaN になる。NaN を「その他」に振り分ける
...
def coarse(p):
    ...
    # 入れてはいけない。NaN にして後段の dropna で除く。
    return None
...
# 「何件除外されたのか」を確かめられなくなる。列を足すだけにして，
# 分析には下の `sl` を使う。
...
4. **コントロール実験**：時代ラベルをシャッフルして同じ手順を実行し，
...
if need(p, '08_word2vec_diachronic.py を先に実行すること'):
    sc = pd.read_csv(p)
...
時代ラベルをシャッフルして同じ手順を実行する。**本物の変化量が
...
| **doc2vec (PV)** | 文書固有のベクトルを学習 | 不安定。ハイパーパラミターに依存 |
...
run_script('09_doc2vec.py', '--chunks', DS/'chunks',
 ...
 ('md', r'''## 1. 何が捉えられているか — カテゴリー効果の比較

`category_effects.csv` は，**同一カテゴリー内の平均類似度**と
**異カテゴリー間の平均類似度**の差を示す。差が大きい軸ほど，
document vector がその軸を強く捉えている。

**作家（`author_ja`）が最大なら要注意。** その document vector が測っているのは
主として「誰が書いたか」であり，時代について論じるには作家の統制が要る。'''),
 ('code', r'''p = D2V/'category_effects.csv'
if need(p, '09_doc2vec.py を先に実行すること'):
    ce = pd.read_csv(p)
    show(ce, caption='カテゴリー効果（同一カテゴリー内 − 異カテゴリー間 の平均類似度）',
         fmt={c: '{:.4f}' for c in ce.columns if ce[c].dtype.kind == 'f'})
    fig, ax = plt.subplots(figsize=(8,4.5))
    ax.barh(ce.field[::-1], ce.gap[::-1], color=PALETTE[0])
    ax.set_xlabel('同一カテゴリー内 − 異カテゴリー間 の平均類似度')
...
if need(p, '09_doc2vec.py を先に実行すること'):
    ...
    # こうしておけば 09 の出力列が変わっても図の描画はエラーで止まらない。
    ...
    print('**そのとき「時代」の残存率が高ければ，時代効果は作家効果と'
          '独立に存在する。** 0 近くまで下がるなら，見ていたのは作家である。')'''),
 ('md', r'''## 3. 分類による評価 — 交差検証
...
- 右：色相は `genre_main` の**2つだけ**にし，下位ジャンルは**マーカーの形**で
  区別する。形は色とは別の視覚的な手がかりなので，色数の上限に縛られない
...
1. `dm=0`（PV-DBOW）と `dm=1`（PV-DM）で学習し，カテゴリー効果を比較すること。
...
- `category_effects.csv` を読み，どのカテゴリーが最も効いているか言える
...
1. LDA の生成過程を説明し，ハイパーパラミターの役割を言える
...
Step 3 でチャンク分割したことがここで生きる。作品を1文書とすると，
...
if need(p, '10_mallet.py を先に実行すること'):
    ts = pd.read_csv(p)
...
2. **固有名詞の束はトピックではない。** 特定作家・特定作品の指紋である。
   `exclusivity` が異常に高いトピックはたいていこれである。
3. **「解釈できないトピック」を認める。** どのモデルにも解釈不能なトピックが1–2割出る。
   無理に解釈しない。'''),
...
学習した。そこには分析の妨げになる語が二種類入る。

| 妨げになる語 | 例 | トピックで起きること |
|---|---|---|
| **固有名詞**（登場人物・地名） | 三吉・岸本・半蔵・法水 | 1作品にしか出ないので，トピックが**作品指標**になる |
...
- **作品とトピックのネットワーク**：作品（丸）とトピック（四角）を，作品のトピック構成（θ の作品平均）で結ぶ。
  どのトピックがどの作品群に担われ，どの作品が複数のトピックにまたがるかが見える
...
1. 本番モデルで，**1作品・1作家に偏ったトピック**（ビューアの ⚠）をいくつか挙げてみよう。
...
7. 「ラベルづけ」で10トピックの依頼文を作り，生成 AI にラベルを付けさせてみよう。
   そのうち3つについて，根拠に挙がった語と作品を詳細と KWIC で確かめ，**種類（主題か指標か）と
   ラベルが妥当か**を判断し，必要なら手で直してみよう。仮ラベルとも比べ，AI が何を補い，
   何を取り違えたかを1段落で述べてみよう
8. 「作品とトピックのネットワーク」で色を「作家」にし，**1人の作家の作品だけが集まるトピック**と，
   **色の違う作品が多く集まるトピック**を1つずつ挙げてみよう。前者は詳細の偏りの警告と一致するか。
   後者の上位語と担う作品を読み，作家をまたぐ主題と言えるかを判断してみよう'''),
 ('md', r'''## 4. 主題の通時変化と多様化
...
直接対応する。エントロピーが高いほど，1つの文書が複数の主題にまたがる。'''),
 ('code', r'''if need(p):
    ...
    # 両端を作品名で出す。エントロピーが低い＝ひとつの主題に集中した
...
if need(p2, '10_mallet.py を先に実行すること'):
    tp = pd.read_csv(p2)
...
### 3. 方法
4つの方法のうち最低2つを使う。ハイパーパラミターと乱数シードを明記する。
