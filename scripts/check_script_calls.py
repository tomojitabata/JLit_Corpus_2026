...
最後のものが 2026-09-24 に実際に起きた。別の作業でできた
`17_delta_workbook.py` `18_pos_select.py` `19_topic_viewer.py` を呼ぶセルが，
古い ``make_notebooks.py`` から作り直したノートブックで**上書きされて消えた**
（git には残っていたので復旧できた）。**消えても何のエラーも出ない**のが
この事故の質である。だから機械に見張らせる。
