...
### 1.1 共用 iMac はホームがマシンをまたがない

DH Lab の iMac は **XCreds** で認証する。どの機体にもログインできるが，
**ホームディレクトリはログインしたマシンにしか作られない。**

> 火曜日に3号機で作った仮想環境と成果物は，木曜日に5号機にログインしても**無い**。

これは故障でも設定ミスでもなく，そういう仕組みである。したがって：

- **どの機体を使ったかを控えておく。** `00_env_check.py` が毎回マシン名を表示する
- **成果物は Git で持ち運ぶ。** ホームに置いたままにしない（§5）
- 機体を移ったら §2 のスクリプトをもう一度実行する（2回目は 5 分で済む）
...
/Users/Shared/jlit/        ← 共用 iMac：このマシンの全員で共有する重い物
   ...
   unidic-novel-v202512/      **本番の辞書**（近現代口語小説UniDic・約 1 GB）
...
- **容量。** UniDic 1 GB ＋ コーパス・モデルで 5–8 GB を見込む。
...
共用 iMac のホームは消える。**成果物は必ず Git に入れて push すること。**
