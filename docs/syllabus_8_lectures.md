...
次にその機体を使う人の処理が遅くなる。KWIC は前の人のサーバが残っていても別のポートを
...
読み書きでき，**同じ機体なら次の受講生が取り直さずに済む**（`00_bootstrap_mac.sh` は
...
提出の書き方は手順書 §5.3。各 Step の課題は型 `my_work/results/StepN_report.md`
（設問・図表・再現のための情報）に**全体で600–1000字程度**で書き，その中身を
Zulip のトピック「Step N」に貼って図（SVG）・表を同じメッセージに添付する（1人1通）。
最終リポートは型 `final_report.md` の構成で**全体で4000–6000字程度**，PDF で提出し，
要旨と PDF を1通で投稿する。再提出は元を直さず同じトピックに新しく投稿する。

型は `scripts/make_notebooks.py` が各 Step の「このステップの課題」の設問から
`templates/` に生成し，`copy_notebooks.py` が受講生の `my_work/results/` に配る。
**設問を直したら make_notebooks.py を走らせ直す**（ノートブックと型が一緒に更新される）。
...
1. **環境構築**（`docs/00_setup_students.md` に従う。最優先）
   ...
   - **辞書は `unidic-novel-v202512`（本番）。** 共用 iMac では，各機体で最初に
     セットアップした人が国立国語研究所から1回だけ取得し（zip 約 1.7 GB），
     その機体の `/Users/Shared/jlit` に置く。同じ機体の2人目以降は
     `00_bootstrap_mac.sh` が辞書の有無を確かめて `[have]` で飛ばす
     （機体どうしでは共有されないので，機体ごとに1回ずつ取得が起きる）
...
### 課題
型 `Step1_report.md` に書き（全体で600–1000字程度），Zulip **2026年度テクスト分析論B ＞ Step 1** に図1枚（**SVG**）とともに1通で投稿。
...
4. `works` で XHTML を取得（**各自，自分の機体で**。1秒/件の間隔で現行64点なら約1分，
   全件で約2分。取得した XHTML はその機体の共有キャッシュにも置かれ，同じ機体での
   2回目以降は青空文庫に取りに行かない）
...
### 最終課題（全体で4000–6000字程度。型 `final_report.md` の構成で書き，PDF で提出）
問いの設定（**答えられない問いも明示**）／データと前処理の決定と理由／
方法（ハイパラと乱数種を明記）／結果（**対照条件・交差検証・安定性検査を必ず含む**）／
考察（作家効果と時代効果の切り分け・偏りの排除・増補すべきテクスト）／
再現のための情報（教材の版・辞書・乱数の種・変えた設定。`config/pipeline.yaml` を添付）。
...
| 回 | 事前準備 | 所要 |
|---|---|---|
| 1 | 共用 iMac に Python 3.12・Homebrew。UniDic は各機体で最初の1人が取得（約 1.7 GB。同じ機体の2人目以降は不要） | 60分 |
| 2 | 回線（XHTML 約104件・数十MB）。各自が自分の機体で取得（1秒/件）。同じ機体の2回目以降は共有キャッシュから | 10分 |
