...
| | 環境 | 特徴 | 教員側の手当て |
|---|---|---|---|
| A | DH Lab 共用 iMac | **ホームがマシンをまたがない**（XCreds）。admin 権限は使わせない | 初回に機体を固定して割り当て，Git 運用を Step 1 で徹底する |
| B | 持ち込みの MacBook | ホームは消えない。Intel 機も混じる | A と同じスクリプトに `--personal` を付けるだけ |
| C | 自分の Windows 11 | 文字コードとパスで詰まる | `PYTHONUTF8=1`，MALLET を `C:\JLit\` に |

A の制約が最も重い。**受講生が「前回の作業が消えた」と言ってきたら，
まず機体を確認する。** `00_env_check.py` が毎回マシン名を表示するので，
レポートに控えさせること。

重い共有物（JDK・MALLET・UniDic 約 1 GB・取得済み XHTML）は
