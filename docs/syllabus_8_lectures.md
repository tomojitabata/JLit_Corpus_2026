...
| | 環境 | 特徴 | 教員側の手当て |
|---|---|---|---|
| A | DH Lab 共用 iMac | **ホームは機体ごとに別々**（XCreds。各機体のホームは保持されるが共有されない）。admin 権限は使わせない | 初回に機体を固定して割り当て，Git 運用を Step 1 で徹底する |
...
### 課題
`results/<名前>/Step1_report.md`（600–1000字）＋図1枚（**SVG**）。
...
## Step 5 word2vec の原理 — 分布仮説から埋め込みへ
...
## Step 6 通時的 word2vec — 時代スライスと Procrustes 整列
